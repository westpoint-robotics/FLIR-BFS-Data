...
int main(int argc, char **argv)
{
    ...
    */
    ...
    //NBL 13 Sept 2019: If record data is TRUE, I don't want
    //to be initializing cameras, or setting acquisition modes, etc.
    //I want to be in (presumably) continuous acquisition until
    //record data gets set to FALSE.
    //If record data does get set to FALSE, I can do clean up
    //for cameras.  I can reinitalize and reset everything
    //at the START of every transition to record data getting
    //set to TRUE.
    //The trick is that, no matter the current setting of record data,
    //the program always has to come back to main() in order to spin
    //as get the next value of record data.  It may be that the best
    //thing to do is check the value of record data after the loop_rate.sleep()
    //call, and say, "if record data is TRUE, and this is the first iteration
    //of this image acquisition process, then do all the initialization and
    //mode setting..."  So, nested if: "if recordData == true, then if
    //first iteration, init and set.  For first, or subsequent iterations,
    //do all the image acquisition stuff."
    //Otherwise, do nothing but check callbacks.
    
    //NBL 13 Sept 2019: This is currently broken code (was not working with FLIR POE
    //reliably, but was working with the USB FLIR).  Modifications to get FLIR POE
    //(which does work with the Command_PPM_Capture code) have gotten the code to a place
    //where it is out of order.
    return 0;
}
